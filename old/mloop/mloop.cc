...
public:
	TaskThread(std::function<void()> task) :
			m_task(std::move(task)), m_wakeup(false), m_stop(false), m_thread(
					&TaskThread::taskFunc, this) {
	}
	~TaskThread() {
		stop();
		join();
...
private:
	...
	void taskFunc() {
		while (true) {
			{
				auto lock = std::unique_lock < std::mutex > (m_wakemutex);
				std::cout << "thread: waiting for wakeup or stop signal..."
						<< std::endl;
			...
			std::cout << "thread: sending task completed signal..."
					<< std::endl;
